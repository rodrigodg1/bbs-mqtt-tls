...
function publish() {
  //var startTime_subA = performance.now()
    ...
    fs.readFile('input_Temp_with_Suburb.json', 'utf8', (err, data) => {
        ...
        fs.appendFile('start_time_subA.csv', String(startTime_subA), function (err) {
            if (err) throw err;
            console.log('Saved!');
        });
        fs.appendFile('start_time_subA.csv', "\n", function (err) {
            if (err) throw err;
            //console.log('Saved!');
        });
  ...
  fs.readFile("input_Temp_with_GPS.json", "utf8", (err, data) => {
    ...
    fs.appendFile("start_time_subB.csv", String(startTime_subB), function (
      err
    ) {
      if (err) throw err;
      console.log("Saved!");
    ...
    const output = execSync(
      "mosquitto_pub -p 8883 -u publisher -P pub --cafile certs/ca/ca.crt --cert client.crt --key client.key -h localhost -f input_Temp_with_GPS.json -t temp_with_gps",
      { encoding: "utf-8" }
    ); // the default is 'buffer'
    

    fs.appendFile("start_time_subB.csv", "\n", function (err) {
      if (err) throw err;
      //console.log('Saved!');
    });
  });
