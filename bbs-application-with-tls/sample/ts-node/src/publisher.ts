...
function publish(){
    ...
    fs.readFile('temperature.json', 'utf8', (err, data) => {
      ...
      //exec publish
      const output = execSync('mosquitto_pub -p 8883 --cafile certs/ca/ca.crt --cert client.crt --key client.key -h localhost -f temperature.json -t temperature', { encoding: 'utf-8' });  // the default is 'buffer'
  ...
  fs.readFile('allfields.json', 'utf8', (err, data) => {
    ...
    //exec publish
    const output = execSync('mosquitto_pub -p 8883 --cafile certs/ca/ca.crt --cert client.crt --key client.key -h localhost -f allfields.json -t allfields', { encoding: 'utf-8' });  // the default is 'buffer'
    //console.log('Output was:\n', output);
